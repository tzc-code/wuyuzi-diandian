...
- `sync.py` rc=0，但 **harvest 全废**：`扫描 25s 未发现 WeChatAppEx 渲染的页面`
  + `微信登录态: **未登录(或仍在登录窗口)**`。
- **确诊根因**：微信主窗口 UIA 读到 `提示` / `你已退出微信` / 按钮 `我知道了·切换账号·仅传输文件`
  ⇒ **微信已退出登录，须人工扫码**。旁证：主窗口仅 `280x380` 且 rect x 为负（屏幕外），
  而登录态正常约 `1079x883` 在屏内；`WeChatAppEx.exe` 仍有 3 个残留进程（进程在 ≠ 可抓取）。
- 重试 2 次（sync 内 1 次 + 单跑 harvest_list.py 1 次）结论一致，按规则停止。
- fetch 77/77 OK（全旧文）→ build 77 → commit `ed766ab`（README 时间戳 + 2 memory 文件）push 成功，
  `HEAD == origin/main == ed766ab`。含本次 README-only 变更属**失败副产物**，非「0 新增正常」。
- 已把该模式写入 skill `wechat-mp-archive-github` 第 9 节「模式 D」。
- **下次跑之前的前置检查（重要）**：不只要看 Weixin.exe 在不在，还要确认**没掉登录**
  （UIA 读主窗口无「你已退出微信」）+ **该号 WebView 面板存在**。二者缺一必失败。
