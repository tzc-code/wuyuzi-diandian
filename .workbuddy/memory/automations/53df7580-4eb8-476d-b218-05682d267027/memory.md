...
### 2026-09-25 22:00-22:06（harvest 连续第 2 天失败，同 09-24 变体）
- 微信运行中（Weixin.exe PID 31284，窗口标题「微信」）；WeChatAppEx.exe 有 3 个进程但均非目标页面。
- harvest 失败：`扫描 25s 未发现 WeChatAppEx 渲染的页面` + `登录态: 未登录(或仍在登录窗口)`。
- 完整 sync.py 跑 2 次（达规程上限），结果一致 → 持续性条件，**新增 0 篇**。
- fetch 77/77 OK（无新增图片）、build 77/77、index 共 77 篇（最新内容仍是 09-24「。。」）。
- git：`6e3046f`（3 项变更，含当日 memory 文件）+ 收尾 `79fb9b6`（README 时间戳），均已 push，`HEAD == origin/main == 79fb9b6`。
- ⚠️ 新踩坑：用 `... | head -30` 跑 sync.py 会因管道提前关闭（SIGPIPE）**截断脚本后半段**，
  导致 git commit/push 未执行、README.md 残留为未提交状态。**以后跑 sync.py 一律重定向到文件或 tail，不要用 head。**
- 待人工：在微信里打开「无语子点点」任意一篇文章（或主页），下轮即可恢复增量抓取。
